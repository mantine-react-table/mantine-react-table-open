...
<Head>
  <title>{'Best Practices (TypeScript) - Mantine React Table Open Docs'}</title>
  <meta
    name="description"
...
```bash
npx npm-check-updates -u mantine-react-table-open @mantine/core @mantine/hooks @mantine/dates @tabler/icons-react dayjs clsx
```
...
/**
 * @type {import('mantine-react-table-open').MRT_ColumnDef<User>[]}
 */
const columns = [
  {
    accessorKey: 'id', //you should get type hints for all of your keys if you defined your TData type correctly
...
```ts
import { type MRT_RowData, type MRT_TableOptions } from 'mantine-react-table-open';
