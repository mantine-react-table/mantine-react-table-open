...
<Head>
  <title>
    {'Global Filtering/Search Feature Guide - Mantine React Table Open Docs'}
  </title>
  <meta
    name="description"
...
A very common use case when you have a lot of data is to filter the data on the server, instead of client-side. In this case you will want to set the `manualFiltering` prop to `true` and manage the `globalFilter` state yourself like so. (Can work in conjuntion with [manual column filtering](https://www.mantine-react-table-open.com/docs/guides/column-filtering#manual-server-side-column-filtering))
...
View Extra Storybook **[Examples](https://www.mantine-react-table-open.dev/?path=/story/features-search-examples)**
